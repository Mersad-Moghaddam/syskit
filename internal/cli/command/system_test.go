...
func TestSystemCommandOutput(t *testing.T) {
	...
	for _, tt := range []struct{ name, format, golden string }{{"table", "table", "system_table.golden"}, {"json", "json", "system_json.golden"}} {
		t.Run(tt.name, func(t *testing.T) {
			cmd := NewSystemCmd(fakeSystemService{info: info}, SystemOptions{Format: func() string { return tt.format }, NoHeader: func() bool { return false }, Color: func() bool { return false }})
